...
/*   Updated: 2024/02/04 17:31:34 by tcharuel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_command_file(const char *path, const char *cmd)
{
	...
	while (paths[i])
	{
		...
		if (!command_file)
			return (perror("minishell"), ft_free_strs(paths), NULL);
...
t_command_status	handle_command(t_state *state, t_command *command)
{
	...
	while (paths[i])
	{
		...
		if (!command_path)
			return (perror("minishell"), ft_free_strs(paths), COMMAND_ERROR);
		if (!access(command_path, X_OK))
			return (ft_free_strs(paths), free(command->argv[0]),
	...
	return (ft_free_strs(paths), COMMAND_NOT_FOUND);
}
